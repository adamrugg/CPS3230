...
try{
RunningClock.start();
pw = new PrintWriter("C:\\Users\\adamr\\workspace\\Task 2/src/login/output_login.txt");
...
int _state_id_badLoginsProperty = 1;

public void _performLogic_badLoginsProperty(String _info, int... _event) {

_cls_login0.pw.println("[badLoginsProperty]AUTOMATON::> badLoginsProperty("+") STATE::>"+ _string_badLoginsProperty(_state_id_badLoginsProperty, 0));
_cls_login0.pw.flush();

if (0==1){}
else if (_state_id_badLoginsProperty==0){
		if (1==0){}
		else if ((_occurredEvent(_event,0/*logOut*/))){
		loggedIn =false ;
_cls_login0.pw .println ("Valid logout");

		_state_id_badLoginsProperty = 1;//moving to state loggedOut
		_goto_badLoginsProperty(_info);
		}
}
else if (_state_id_badLoginsProperty==1){
		if (1==0){}
		else if ((_occurredEvent(_event,4/*invalidLogin*/))){
		loggedIn =false ;
_cls_login0.pw .println ("Invalid login");

		_state_id_badLoginsProperty = 1;//moving to state loggedOut
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,2/*validLogin*/))){
		loggedIn =true ;
_cls_login0.pw .println ("Valid login");

		_state_id_badLoginsProperty = 0;//moving to state loggedIn
		_goto_badLoginsProperty(_info);
		}
}
}
...
_cls_login0.pw.println("[badLoginsProperty]MOVED ON METHODCALL: "+ _info +" TO STATE::> " + _string_badLoginsProperty(_state_id_badLoginsProperty, 1));
_cls_login0.pw.flush();
}

public String _string_badLoginsProperty(int _state_id, int _mode){
switch(_state_id){
case 0: if (_mode == 0) return "loggedIn"; else return "loggedIn";
case 1: if (_mode == 0) return "loggedOut"; else return "loggedOut";
